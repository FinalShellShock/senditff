...
// where you go shopping. Pure UI over positionScores + teamState.
...
// production is the one thing it should not be shopping for.
const PUNTING_STATES: TeamState[] = ["REBUILD", "EARLY_REBUILD", "STUCK"];

function isBuyer(p: TeamProfile): boolean {
  return !PUNTING_STATES.includes(p.teamState);
}
...
function classOf(p: TeamProfile, pos: Position): SubClassification {
  ...
  return (
    ps?.starterClassification ??
    (ps?.classification === "CRITICAL_NEED"
      ? "CRITICAL"
      : ((ps?.classification as SubClassification) ?? "HEALTHY"))
  );
}
  ...
  if (mine) {
    for (const pos of POSITIONS) {
      ...
      const desperate = profiles
        .filter(
          (p) => !p.isMine && classOf(p, pos) === "CRITICAL" && isBuyer(p),
        )
        .sort(
          (a, b) =>
            (a.positionScores?.[pos]?.starterValue ?? 0) -
            (b.positionScores?.[pos]?.starterValue ?? 0),
        );
      for (const target of desperate.slice(0, 2))
        callouts.push({ pos, target });
    }
  }
      ...
      <div className="lb-columns">
        {POSITIONS.map((pos) => {
          ...
          const maxValue = Math.max(
            1,
            ranked[0]?.positionScores?.[pos]?.starterValue ?? 1,
          );
          return (
            <div key={pos} className="lb-col">
              ...
              {ranked.map((p, i) => {
                ...
                const seller =
                  !hasHole &&
                  (cl === "SURPLUS" || ps?.depthClassification === "SURPLUS");
                return (
                  <div
                    key={p.rosterId}

...
Wired in via `src/data/qbSignals.json`, generated by
`node scripts/research/build-qb-signals.mjs`. Joined onto players in
`api/_lib/buildTeams.ts` by normalized name, the same seam FantasyCalc values
already use, and consumed through `effectiveAge()` in `src/algo/profile.ts`.

**Sack rate was deliberately NOT shipped**, despite testing as strong as
rushing share. Applied to the live QB population it flags Zach Wilson, Will
Levis, Caleb Williams and Bryce Young: young QBs on bad teams, not aging ones.
The research controlled for quality by stratifying on current production;
applying the raw signal to real rosters throws that control away and the
confound walks straight back in. Rushing share survives because it describes a
STYLE, not a quality tier.

The signal knocks down remaining career value, then reads back the age that
normally corresponds to it, so everything downstream (window pressure, the
projection, the age-arb gates) works unchanged on one number.

Two guards:
- Ramps in from age 26 to 30, matching the 27-32 range actually measured. A
  narrow 26-28 ramp made Jalen Hurts jump five effective years between two
  birthdays, an artifact of the flat QB curve rather than a real cliff.
- Effective age never goes below calendar age. Signals are warnings only.

Live effect on Johnny's league: Josh Allen 30.2 -> 33.3 effective, Lamar
Jackson 29.5 -> 33.1, Jalen Hurts 28.0 -> 31.0. Jayden Daniels (25.6) and
Drake Maye (23.9) are flagged rushers but below the ramp, so untouched.
Burrow, Herbert, Mahomes and Purdy carry no signal and are unchanged.

## Earlier note on plumbing

The blocker was data, not statistics: the app knows each player's name,
position, age and value from Sleeper and FantasyCalc, but nothing about career
rushing share.
